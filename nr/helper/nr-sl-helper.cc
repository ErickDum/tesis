                                       ...
                                       const std::set<uint8_t>& slBwpIds)
{
    ...
    for (const auto& itBwps : slBwpIds)
    {
        ...
        Ptr<NrSlChunkProcessor> pSlSinrEst = Create<NrSlChunkProcessor>();
        pSlSinrEst->AddCallback(MakeCallback(&NrSpectrumPhy::UpdateSlSinrEstPerceived, spectrumPhy));
        spectrumPhy->AddSlSinrEstChunkProcessor(pSlSinrEst);
