...
{
    //std::cout << "Antes suma dentro " << *params->psd << std::endl;
    NS_LOG_FUNCTION(this);
    ...
    //std::cout << "nr-spectrum-phy.cc: StartRx" << std::endl;
...
void
NrSpectrumPhy::AddSlSinrEstChunkProcessor(Ptr<NrSlChunkProcessor> p)
{
    NS_LOG_FUNCTION(this);
    m_slInterference->AddSinrEstChunkProcessor(p);
}
...
void
NrSpectrumPhy::UpdateSlSinrEstPerceived(std::vector<SpectrumValue> sinr_est)
{
    NS_LOG_FUNCTION(this);
    m_slSinrEstPerceived = sinr_est;

    //std::cout << "nr-spectrum-phy.cc3: ";
    //for (const auto& val : sinr_est)
    //{
        //std::cout << val << " ";
    //}
    //std::cout << std::endl;

    Ptr<NrUePhy> uePhy = DynamicCast<NrUePhy>(GetPhy());
    uePhy->UpdateSinrEst(sinr_est);
}
...
NrSpectrumPhy::RxSlPssch(std::vector<uint32_t> paramIndexes)
{
    ...
    for (uint32_t i = 0; i < paramIndexes.size(); i++)
    {
        ...
        /*
        std::cout << "m_sinrPerceived values for RNTI " << tag.GetRnti() << ": ";
        for (auto it = itTb->second.m_sinrPerceived.ConstValuesBegin();
            it != itTb->second.m_sinrPerceived.ConstValuesEnd(); ++it)
        {
            std::cout << *it << " ";
        }
        std::cout << std::endl;
        ...
        if (m_slDataErrorModelEnabled)
        {
            ...
            tbIt.second.m_outputEmForData =
                m_slErrorModel->GetTbDecodificationStats(tbIt.second.m_sinrPerceived,
                                                         ...
                                                         harqInfoList);
            //std::cout << "error" << tbIt.second.m_sinrPerceived << std::endl;                                                         
            if (!rbCollided)
            {
