...
void 
NrUePhy::UpdateSinrEst(std::vector<SpectrumValue> sinr_est){
    m_sinrEst = sinr_est;
}
...
NrUePhy::PhyPscchPduReceived(const Ptr<Packet>& p, const SpectrumValue& psd)
{
    ...
    //uint32_t psdSize = psd.GetSpectrumModel()->GetNumBands();

    double rsrpDbm = GetSidelinkRsrp(psd).second;
    double rssiDbm = GetSidelinkRssi(psd, m_sinrEst[0]);
    double rsrqDbm = 10 * log10(numRB(psd)) + rsrpDbm - rssiDbm;
    double rsrqLinear = pow(10.0, (rsrqDbm / 10.0));
    ...
    for (int i = 0; i < numSubcanales; ++i) {
        double power = 0.0;
        int offset = i * rbPorSubcanal;
        for (int j = 0; j < rbParaPotencia; ++j) {
            power += m_sinrEst[0][offset + j];
        }
        powers.push_back(power);
    }

    if (m_netDevice->GetNode()->GetId() == 9999)
    {
        //std::cout << "Nodo actual: " << m_netDevice->GetNode()->GetId() << std::endl;
        //std::cout << "--------------" << std::endl;

        //std::cout << "aPa:" << m_sinrEst[0] << std::endl;
        //std::cout << "aQa:" << psd << std::endl;
    }
...
double
NrUePhy::GetSidelinkRssi(SpectrumValue psd, SpectrumValue interf){
    double rssiW = 0.0; 
    uint16_t rbCount = 0;

    for (Values::const_iterator itSig = psd.ConstValuesBegin(),
                                itInt = interf.ConstValuesBegin();
        itSig != psd.ConstValuesEnd()
        && itInt != interf.ConstValuesEnd();
        ++itSig, ++itInt)
    {
        if ((*itSig))
        {
            uint32_t scSpacing = 15000 * (1 << GetNumerology()); // eq. 2^GetNumerology()
            uint32_t rbWidthInHz = scSpacing * NrSpectrumValueHelper::SUBCARRIERS_PER_RB;

            double powerSignalRb = ((*itSig) * rbWidthInHz);  // [W]
            double powerInterfRb = ((*itInt) * rbWidthInHz);  // [W]

            double powerTotalRb = powerSignalRb + powerInterfRb;

            // posiblemente multiplicar por 2 o 3 de las subportadoras con dmrs pero no se xd
            rssiW += powerTotalRb;
            rbCount++;
        }
    }

    double rssiDbm = 10.0 * log10(1000.0 * rssiW);

    return rssiDbm;
}


int NrUePhy::numRB(SpectrumValue psd)
{
    uint16_t numRB = 0;
    for (Values::const_iterator itPi = psd.ConstValuesBegin(); itPi != psd.ConstValuesEnd(); itPi++)
    {
