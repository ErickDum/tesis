...
class NrSlUeMacSchedulerFixedMcs : public NrSlUeMacScheduler
{
  ...
  private:
    ...
    /**
     * \brief Select the resources with the least power measured for a grant from the candidate resources
     *
     * If K denotes the candidate resources, and N_PSSCH_maxTx is the
     * maximum number of PSSCH configured transmissions, then:
     *
     ...
     */
    std::list<SlResourceInfo> SelectResourcesBasedOnPower(
        std::list<SlResourceInfo> txOpps);

    /**
     * \brief Select the resources with a weight calculated inversly proportional to the power measured
     *
     * If K denotes the candidate resources, and N_PSSCH_maxTx is the
     * maximum number of PSSCH configured transmissions, then:
     *
