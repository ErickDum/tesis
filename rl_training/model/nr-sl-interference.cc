...
NrSlInterference::DoDispose()
{
    ...
    m_sinrestChunkProcessorList.clear();
...
NrSlInterference::StartRx(Ptr<const SpectrumValue> rxPsd)
{
    ...
    for (std::list<Ptr<NrSlChunkProcessor>>::const_iterator it = m_sinrestChunkProcessorList.begin();
         it != m_sinrestChunkProcessorList.end();
         ++it)
    {
        (*it)->Start(init);
    }
...
NrSlInterference::EndRx()
{
    ...
    else
    {
        ...
        for (std::list<Ptr<NrSlChunkProcessor>>::const_iterator it =
                 m_sinrestChunkProcessorList.begin();
             it != m_sinrestChunkProcessorList.end();
             ++it)
        {
            (*it)->End();
    ...
    if (m_receiving && (Now() > m_lastChangeTime))
    {
        ...
        for (uint32_t index = 0; index < m_rxSignal.size(); ++index)
        {
            ...
            for (std::list<Ptr<NrSlChunkProcessor>>::const_iterator it =
                 ...
                 ++it)
            {
                (*it)->EvaluateChunk(index, sinr_est, duration);
            }
            for (std::list<Ptr<NrSlChunkProcessor>>::const_iterator it =
                     m_interfChunkProcessorList.begin();
                 it != m_interfChunkProcessorList.end();
...
void
NrSlInterference::AddSinrEstChunkProcessor(Ptr<NrSlChunkProcessor> p)
{
    NS_LOG_FUNCTION(this << p);
    m_sinrestChunkProcessorList.push_back(p);
}
