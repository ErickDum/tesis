    ...
    static TypeId tid =
        TypeId("ns3::NrSlUeMac")
            ...
            .AddAttribute("EnableSensing",
                          ...
                          MakeBooleanChecker())
            .AddAttribute("SensingMethod",
                          "Sensing method to be used for NR Sidelink resource selection",
                          UintegerValue(0),
            ...
            .AddAttribute("ResourceAllocationMethod",
                          ...
                          MakeUintegerAccessor(&NrSlUeMac::SetResourceAllocationMethod,
                                               &NrSlUeMac::GetResourceAllocationMethod),
            ...
            .AddAttribute(
                "SlThresPsschPower",
                "A threshold in W used for sensing based UE autonomous resource selection",
                DoubleValue(1.875e-17),
                MakeDoubleAccessor(&NrSlUeMac::SetSlThresPsschPower,
                                   &NrSlUeMac::GetSlThresPsschPower),
                                        ...
                                        const std::list<SfnSf>& transmitHistory) const
{
    ...
    report.m_sensingMethod = m_sensingMethod;
    report.m_resourceAllocationMethod = m_resourceAllocationMethod;
    ...
    report.m_PowerThreshold = m_thresPower;
    ...
    std::map<uint32_t, std::vector<double>> aggPowers;
    std::map<uint32_t, std::vector<double>> aggSinr;

    for (const auto& proj : sensingDataProjections)
    {
        for (const auto& res : proj)
        {
            uint32_t sfn = res.sfn.Normalize();

            // Aseguramos entrada en aggPowers
            auto& powerVec = aggPowers[sfn];
            if (powerVec.size() != totalSubCh) {
                powerVec = std::vector<double>(totalSubCh, 0.0);
            }

            // Sumamos potencia por subcanal
            for (size_t i = 0; i < res.slpowers.size(); ++i)
            {
                if (i < totalSubCh) {
                    powerVec[i] += res.slpowers[i];
                }
            }

            // Agregamos sinr_est (tipo double)
            aggSinr[sfn].push_back(res.sinr_est);
        }
    }

    auto it = remainingCandidates.begin();
    while (it != remainingCandidates.end())
    {
        uint32_t sfn = it->sfn.Normalize();

        // 1) Verificamos si el SFN está en aggPowers
        auto mapIt = aggPowers.find(sfn);
        if (mapIt == aggPowers.end())
        {
            it = remainingCandidates.erase(it);
            continue;
        }

        const std::vector<double>& powers = mapIt->second;
        size_t start  = it->slSubchannelStart;
        size_t length = it->slSubchannelLength;

        // 2) Verificamos si el rango es válido
        if (start + length > powers.size())
        {
            it = remainingCandidates.erase(it);
            continue;
        }

        // 3) Promedio de potencia
        double powerSum = 0.0;
        for (size_t j = start; j < start + length; ++j)
        {
            powerSum += powers[j];
        }
        double avgPower = powerSum / static_cast<double>(length);
        it->slPower = avgPower;

        // 4) Promedio de SINR
        auto sinrIt = aggSinr.find(sfn);
        if (sinrIt != aggSinr.end() && !sinrIt->second.empty())
        {
            const std::vector<double>& sinrValues = sinrIt->second;
            double sinrSum = 0.0;
            for (double val : sinrValues)
            {
                sinrSum += val;
            }
            double avgSinr = sinrSum / static_cast<double>(sinrValues.size());
            it->slSinr = avgSinr;  // Asegúrate que slSinr esté definido en la estructura
        }
        else
        {
            // Si no hay SINR disponibles, puedes asignar un valor inválido o predeterminado
            it->slSinr = -999.0;
        }
    ...
    double powerThreshold = m_thresPower;
    report.m_initialRsrpThreshold = m_thresRsrp;
    auto candidateResourcesAfterStep5 = remainingCandidates;

    // Insert this code before the switch(m_sensingMethod), e.g., just after candidateResourcesAfterStep5 is obtained:
    /*
    for (auto & candidate : candidateResourcesAfterStep5)
    {
        double maxCandidatePower = 0.0;
        // Iterate through each list of sensing data projections
        for (const auto & projList : sensingDataProjections)
        {
            // Check if the candidate's slot (normalized) matches that of the sensing projection
            if (!projList.empty() && candidate.sfn.Normalize() == projList.front().sfn.Normalize())
            {
                // Iterate over the subchannels of the candidate
                for (uint8_t i = 0; i < candidate.slSubchannelLength; ++i)
                {
                    uint8_t idx = candidate.slSubchannelStart + i;
                    // Make sure the index is valid in the sensed power vector
                    if (idx < projList.front().slpowers.size())
                    {
                        maxCandidatePower = std::max(maxCandidatePower, projList.front().slpowers[idx]);
                    }
                }
            }
        }
        candidate.slPower = maxCandidatePower;
    }*/

    // Now, update remainingCandidates with the candidates that have been updated
    remainingCandidates = candidateResourcesAfterStep5;

    switch (m_sensingMethod)
    {
    case 0:
        // Sensing method 0: RSRP threshold
        NS_LOG_DEBUG("Sensing method 0: RSRP threshold");
        do
        {
                        ...
                        for (const auto& itReservedResourceProjection : itSensingDataProjections)
                        {
                                ...
                                itReservedResourceProjection.sfn.Normalize())
                            {
                                ...
                                if (itReservedResourceProjection.slRsrp > rsrpThreshold)
                                {
                                    ...
                                    {
                                        NS_LOG_DEBUG("Overlapped resource Recursos1: "
    ...
    case 1:
        // Sensing method 1: Power threshold
        NS_LOG_DEBUG("Sensing method 1: Power threshold");
        // following assignment is needed since we might have to perform
        remainingCandidates = candidateResourcesAfterStep5;
        NS_LOG_DEBUG("Step 6 loop iteration checking " << remainingCandidates.size()
                                                    << " resources against threshold "
                    ...
                    for (const auto& itReservedResourceProjection : itSensingDataProjections)
                    {
                            ...
                            itReservedResourceProjection.sfn.Normalize())
                        {
                            ...
                            if (OverlappedResource(itReservedResourceProjection.sbChStart,
                                                ...
                                                itCandidate->slSubchannelLength))
                            {
                                for (uint8_t i = 0; i < itCandidate->slSubchannelLength; i++)
                                {
                                    if (itReservedResourceProjection.slpowers[i+(itCandidate->slSubchannelStart)] > powerThreshold)
                                    {
                                        ...
                                        NS_LOG_DEBUG("Resource "
                                                    ...
                                                    << (itCandidate->slSubchannelStart +
                                                        itCandidate->slSubchannelLength - 1)
                                                    << "] erased. Its power : "
                                                    << itReservedResourceProjection.slpowers[i]
        ...
        break;
    //case 2:
        // Sensing method 2: SINR threshold
        //NS_LOG_DEBUG("Sensing method 2: RSRP and power threshold");
        //break;
    //default:
...
void
NrSlUeMac::SetsensingMethod(uint8_t sensingMethod)
{
    NS_LOG_FUNCTION(this << +sensingMethod);
    m_sensingMethod = sensingMethod;
}

uint8_t
NrSlUeMac::GetsensingMethod() const
{
    NS_LOG_FUNCTION(this);
...
void
NrSlUeMac::SetResourceAllocationMethod(uint8_t resourceAllocationMethod)
{
    NS_LOG_FUNCTION(this << +resourceAllocationMethod);
    m_resourceAllocationMethod = resourceAllocationMethod;
}

uint8_t
NrSlUeMac::GetResourceAllocationMethod() const
{
    NS_LOG_FUNCTION(this);
    return m_resourceAllocationMethod;
}
...
void
NrSlUeMac::SetSlThresPsschPower(double thresPower)
{
    NS_LOG_FUNCTION(this);
    m_thresPower = thresPower;
}

double
NrSlUeMac::GetSlThresPsschPower() const
{
    NS_LOG_FUNCTION(this);
    return m_thresPower;
}
