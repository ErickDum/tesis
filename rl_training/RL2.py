...
if __name__ == '__main__':
    ...
    # ns3-gym parameters
    ports = [5555, 5556]  # Dos puertos para dos instancias ns-3
    stepTime = 0.1
    seed = 0
    simTime = 40
    simArgs = {"--simTime": simTime}
    envs = [ns3env.Ns3Env(port=p, stepTime=stepTime, startSim=bool(args.start), simSeed=seed, simArgs=simArgs) for p in ports]
    for env in envs:
        env.reset()
    agent = ResourceAllocation(envs[0])  # Usamos la clase existente, pero modificamos el bucle de entrenamiento abajo

    # Modificamos el entrenamiento para controlar ambos entornos
    def multi_env_train(agent, envs, episodes):
        num_states = 4
        num_actions = 4*140*15
        l1, l2, l3 = 512, 256, 128
        policy_dqn = DQN(num_states, l1, l2, l3, num_actions)
        target_dqn = DQN(num_states, l1, l2, l3, num_actions)
        target_dqn.load_state_dict(policy_dqn.state_dict())
        agent.optimizer = torch.optim.Adam(policy_dqn.parameters(), lr=agent.learning_rate_a)
        ...
        for episode in range(episodes):
            ...
            state_list = [torch.tensor(obs, dtype=torch.float32) for obs in obs_list]
            total_reward = 0
            done_list = [False for _ in envs]
            while not all(done_list):
                for i, env in enumerate(envs):
                    ...
                    if random.random() < epsilon:
                        ...
                        with torch.no_grad():
                            q_vals = policy_dqn(state_list[i])
                            action = q_vals.argmax().item()
                    print(f"Env {i} Accion: {action}")
                    next_obs, reward, done, info = env.step(action)
                    if next_obs is None:
                        done_list[i] = True
                        continue
                    next_state = torch.tensor(next_obs, dtype=torch.float32)
                    total_reward += reward
                    memory.append((state_list[i], action, next_state, reward, done))
                    state_list[i] = next_state
                    done_list[i] = done
                    step_count += 1
                if len(memory) > agent.mini_batch_size:
                    batch = memory.sample(agent.mini_batch_size)
                    agent.optimize(batch, policy_dqn, target_dqn)
    ...
    rewards, eps_history = multi_env_train(agent, envs, args.iterations)
